...
test.describe('Accessibility (axe-core)', () => {
	test('home page has no critical violations', async ({ page }) => {
		await page.goto('/')
		await expect(page.locator('#main-content h1')).toBeVisible({ timeout: 20_000 })
	...
	test('bridge page has no critical violations', async ({ page }) => {
		...
		await expect(
			page.locator('#main-content').getByRole('heading', { level: 1, name: 'USDC Bridge' }),
...
test.describe('Keyboard navigation', () => {
	...
	test('keyboard-only: connect, fill form, routes or result visible', async ({ page }) => {
