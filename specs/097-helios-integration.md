...
- **In-browser (@a16z/helios):** `createHeliosProvider(config, kind)` with `kind`: `HeliosNetworkKind` (`Ethereum` | `Opstack` | `Linea`) from `src/constants/helios-chains.ts`. Returns EIP-1193 provider; must call `waitSynced()` before use. Config: `executionRpc`, `consensusRpc` (ethereum/opstack), optional `checkpoint`, `network`, `dbType` ("localstorage" | "config"). WASM is loaded automatically by the package.
...
- [x] Supported chains for Helios are defined (Ethereum mainnet at minimum; optionally sepolia, OP Mainnet, Base, Linea as per Helios support). `src/constants/helios-chains.ts`: `HeliosNetworkKind` enum (Ethereum, Opstack, Linea), `HELIOS_CHAINS` (ChainId → network + kind), `HELIOS_SUPPORTED_CHAIN_IDS`, `isHeliosSupportedChain()`, `getHeliosChainInfo()`; Ethereum, EthereumSepolia, Optimism, OPSepolia, Base, BaseSepolia, WorldChain, WorldChainSepolia, Unichain, UnichainSepolia, Linea, LineaSepolia. `DataSource.Helios` added.
...
- [x] **Browser mode (optional):** When "Use Helios (browser)" is enabled for a supported chain, the app creates (or reuses) a Helios provider via `@a16z/helios`, waits for sync, and uses it as the VoltaireProvider for that chain. Sync state is visible; fallback on failure.
...
Complete. 2026-02-21 (PROMPT_build): Browser mode implemented. `@a16z/helios` optional dep; `src/lib/helios-rpc.ts`: `HeliosBrowserSyncStatus` enum (Idle, Syncing, Ready, Fallback), `heliosBrowserEnabled`, `setHeliosBrowserEnabled`, `getHeliosBrowserEnabled`, `getHeliosBrowserSyncStatus`, `setHeliosBrowserSyncStatusHandler`; `getOrCreateHeliosBrowserProvider(chainId)` dynamic-imports Helios, `createHeliosProvider` + `waitSynced()`, caches by (chainId, configHash), wraps as VoltaireProvider with fallback on request/sync failure; `createProviderForChain` returns lazy wrapper when browser enabled. Network page shows "Use Helios (browser)" toggle and sync status (syncing… / ready / fallback) for Helios-supported chains. Layout fallback notice covers both local and browser.
