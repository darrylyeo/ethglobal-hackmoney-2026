...
Complete. DataSource enum (Local, LiFi, TokenLists, Stork, Uniswap, Yellow, Cctp, Voltaire, PartyKit, Covalent). All collections have $source; transfer-graphs collection; USDC page (`/explore/usdc`) uses collection; CctpFees/CctpAllowance filter by source. Balances.svelte shows data-balances-grid when connected; CoinAmountInput range slider has aria-label for a11y; e2e fixes (chain select focus+ArrowDown/Enter, amount textbox selector, protocol badge selector). `deno task test:e2e` passes.
