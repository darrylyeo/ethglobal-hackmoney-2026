...
- When the state/search params contain action slugs (or are empty), **do not persist yet**.
  ...
  search params: `actions` and `params`, and an autogenerated `name` from the first action’s label (e.g. Swap,
  Bridge).
...
- Session navigation items resolve to `/session?template=[action-slug]`.
