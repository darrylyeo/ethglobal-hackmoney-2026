...
- Session initialization comes from the hash:
  - `/session#/[action-slug]` (e.g. `#/swap`, `#/bridge`)
  - `/session#/[action-1-slug]|[action-2-slug]`
  - `/session#/[action-1-slug]:{JSON params}|[action-2-slug]:{JSON params}`
- When the hash contains action slugs (or is empty), **do not persist yet**.
  Build an ephemeral session in memory from the hash: `actions` and `params` from
  the hash, and an autogenerated `name` from the first action’s label (e.g. Swap,
  Bridge). The URL hash stays unchanged.
- **Persist on first touch:** When the user first interacts with the form (focus
  ...
  replace the hash with `#/session:<id>`.
- If there is no hash (or empty hash), use the same ephemeral behaviour with a
...
- If the hash matches `#/session:<id>`, load the session from the collection and
