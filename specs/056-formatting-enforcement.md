...
**One file per run.** Each run of this spec must touch exactly one file: format that file to repo preferences (including Svelte section spacing: two empty lines between sections and between comment groups in `<script>`—manually enforced), then check its box. Do not format or verify multiple files in a single run.
...
All files below: one file per run; format to repo preferences (section spacing manually enforced), then check.
...
- [x] `e2e/support/coverage-helpers.ts`
- [x] `e2e/support/coverage-manifest.ts`
- [x] `e2e/support/coverage-utils.ts`
...
- [x] `scripts/assets/sync.ts`
- [x] `scripts/_yellow-demo.ts`
- [x] `scripts/checks/check-bundle-size.mjs`
- [x] `scripts/checks/check-performance.mjs`
...
- [x] `src/data/Session.ts`
- [x] `src/data/SessionSimulation.ts`
