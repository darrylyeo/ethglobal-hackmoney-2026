...
Implemented. Network, Actor, Contract, Block, Coin, ActorCoin, ChainTransaction, TransactionTrace, VerifiedContractSource, TokenListCoin, ActorAllowance, EvmActorProfile, TransferEvent, EnsAvatar, StorkPrice use nested `$id`; GraphScene Network node id uses `row.$id.chainId`; collections and call sites updated.
