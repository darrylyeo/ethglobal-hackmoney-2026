...
- **Agent chat:** `createLlmProvider` (and explain/agent-chat flow) accepts optional connection id (and optional model id); when provided, use that connection’s provider and API key; otherwise keep current fallback chain (prompt-api → hosted → zen).
