...
- e2e: `src/routes/page.test.ts` expected
  `getByRole('option', { name: 'Ethereum' })` for To chain; Bits UI Select does
  not expose native option role. Fixed test to use
  `getByTestId('option-Ethereum')` and `getByTestId('option-OP Mainnet')`.
...
- `git pull --rebase origin main` hit merge conflicts (deno.json, deno.lock,
  package.json, specs).
