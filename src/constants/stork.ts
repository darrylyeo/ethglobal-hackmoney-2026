...
export const storkEndpointsByRegion = Map.groupBy(
	storkEndpoints,
	(e) => e.region,
)
...
export const storkWebsocketUrls = Object.fromEntries(
	storkEndpoints
		.filter(
			(endpoint) => endpoint.transportType === StorkTransportType.WebSocket,
		)
		.map((endpoint) => [endpoint.region, endpoint.url]),
) satisfies Record<StorkRegion, string>
...
export const storkEncodedAssetIdByAssetId = Object.fromEntries(
	storkAssetEntries.map((asset) => [asset.assetId, asset.encodedAssetId]),
)
...
const storkPushedNetworkEnvToChainId: Record<
	string,
...
const storkAssetChainEntries = Object.entries(
	storkPushedAssetsByNetwork,
).flatMap(([networkName, envs]) =>
...
export const storkAssetEntriesWithMeta: readonly StorkAssetDefinition[] =
	storkAssetEntries.map((e) => ({
		...e,
...
export const storkAssetsByAssetId: Record<string, StorkAssetDefinition> =
	Object.fromEntries(storkAssetEntriesWithMeta.map((a) => [a.assetId, a]))
