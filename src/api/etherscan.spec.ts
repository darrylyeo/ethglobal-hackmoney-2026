...
import { fetchAbiFromEtherscan } from './etherscan.ts'
import { ChainId } from '$/constants/chain-id.ts'

const address = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48' as `0x${string}`

Deno.test('fetchAbiFromEtherscan: unsupported chain returns null without calling fetch', async () => {
	const out = await fetchAbiFromEtherscan(ChainId.XDC, address)
	assertEquals(out, null)
})

Deno.test('fetchAbiFromEtherscan: status 1 and valid result JSON returns ABI array', async () => {
	const abi = [{ type: 'function', name: 'balanceOf', inputs: [{ name: 'account', type: 'address' }] }]
	...
	try {
		const out = await fetchAbiFromEtherscan(ChainId.Ethereum, address)
...
Deno.test('fetchAbiFromEtherscan: status 0 returns null', async () => {
	const originalFetch = globalThis.fetch
	globalThis.fetch = async (input: RequestInfo | URL) => {
		const url = typeof input === 'string' ? input : input instanceof Request ? input.url : (input as URL).href
		if (!url.includes('getabi')) return new Response('', { status: 404 })
