...
Deno.test(
	'encodeBalanceOfCall: produces balanceOf selector + encoded address',
	() => {
		const account =
...
Deno.test(
	'getErc20Balance: calls eth_call with to and encoded data, returns decoded balance',
	async () => {
...
Deno.test(
	'getErc20Balance: returns 0n for empty or short eth_call response',
	async () => {
		const provider: VoltaireProvider = {
			request: async () => '0x',
			on: () => provider,
