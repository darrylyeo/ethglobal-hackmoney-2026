/**
 * Signature database — 4byte.sourcify.dev (replaces openchain.xyz, same API).
 * https://4byte.sourcify.dev/
 */

const SOURCIFY_4BYTE_BASE = 'https://api.4byte.sourcify.dev/signature-database/v1'
...
export async function fetchFunctionSignatures(selector: `0x${string}`) {
	...
	const res = await fetch(`${SOURCIFY_4BYTE_BASE}/lookup?function=${key}`)
	if (!res.ok) return []
	const json = (await res.json()) as { ok?: boolean; result?: LookupResult }
...
export async function fetchEventSignatures(topicHash: `0x${string}`) {
	...
	const res = await fetch(`${SOURCIFY_4BYTE_BASE}/lookup?event=${key}`)
	if (!res.ok) return []
	const json = (await res.json()) as { ok?: boolean; result?: LookupResult }
