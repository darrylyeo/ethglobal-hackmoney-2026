...
test('after networks load, Networks section lists Ethereum with ID 1', async ({ page }) => {
	await page.goto('/test/collections')
	await page.getByText('Loading networks…').waitFor({ state: 'hidden', timeout: 10_000 })
	await expect(page.getByRole('heading', { level: 2, name: 'Networks' })).toBeVisible()
	await expect(
		page.locator('section:has(h2:has-text("Networks")) ul li').filter({ hasText: 'Ethereum' }),
...
test('after coins load, USDC coins section lists USDC on chain 1 with address', async ({
	...
	await page.goto('/test/collections')
	await page.getByText('Loading coins…').waitFor({ state: 'hidden', timeout: 10_000 })
	await expect(page.getByRole('heading', { level: 2, name: 'USDC coins' })).toBeVisible()
	const firstCoin = page.locator('section:has(h2:has-text("USDC coins")) ul li').first()
