...
>
	...
	>
		...
		<p data-text="muted">
			Drag balances into from/to slots to resolve intents.
		</p>
	</section>
	...
	>
		<h2>
			Registered intents ({intents.length})
		</h2>
		<div data-column>
		...
		<div data-row="gap-3 wrap">
			...
			>
				...
				{#if fromPayload}
					<pre
						data-text="muted"
						data-testid="from-payload"
					>{JSON.stringify(fromPayload, null, 2)}</pre>
				{:else}
					<p data-text="muted">
						Drop an entity
					</p>
	...
	>
		...
		{#if resolution?.matched}
			...
			>
				...
				{#if resolution.error}
					<p data-text="muted">
						Error: {resolution.error instanceof Error ? resolution.error.message : String(resolution.error)}
					</p>
		...
		{:else if resolution}
			<p data-text="muted">
				No matching intent for these entity types.
			</p>
		{:else}
			<p data-text="muted">
				Select two entities.
