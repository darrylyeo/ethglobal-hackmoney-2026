<script lang="ts">
	...
	} from '../about-old/architecture-graph.ts'
	...
	import { getArchitectureGraphModel } from '../about-old/architecture-graph.ts'
...
<div class="architecture-graph-wrap">
	...
	<div
		role="application"
		tabindex="0"
		aria-label="Architecture diagram of app systems, data flows, and dependencies"
		...
		{@attach (container) => {
			...
			const optionalStroke = prefersDark ? '#94a3b8' : '#cbd5e1'
			...
			> = prefersDark
				? {
						...
						config: { fill: '#115e59', stroke: '#2dd4bf' },
				...
				: {
						...
						config: { fill: '#99f6e4', stroke: '#2dd4bf' },
			...
			const getNodeStyle = (n: ArchitectureNode): NodeData['style'] => {
				...
				return n.image
					...
					: {
							...
							fill: base.fill,
							stroke: base.stroke,
							lineWidth: n.importance && n.importance >= 4 ? 2 : 1,
							labelText: n.label,
							labelBackground: true,
							labelBackgroundFill: labelBg,
							labelBackgroundRadius: 6,
			...
			const buildData = () => ({
				nodes: model.nodes.map((n) => ({
					id: n.id,
					combo: comboByArea.has(n.area) ? `combo-${n.area}` : undefined,
					type: getNodeType(n),
					data: {
						label: n.label,
						category: n.category,
				...
				combos: model.combos.map((combo) => ({
					...
					style: {
						...
						fill: categoryStyles[combo.category].fill,
						fillOpacity: prefersDark ? 0.08 : 0.18,
						stroke: categoryStyles[combo.category].stroke,
						lineWidth: 1,
						lineDash: combo.category === 'tooling' ? [6, 4] : undefined,
			...
			graph = new Graph({
				...
				behaviors: [
					{ type: 'drag-canvas' },
					{ type: 'zoom-canvas', animation: !reducedMotion },
					{ type: 'drag-element', dropEffect: 'link' },
					{ type: 'click-select' },
					{ type: 'hover-activate' },
					{ type: 'focus-element' },
					{ type: 'brush-select', enableElements: ['node', 'edge'] },
	...
	>
		{#if hoveredId}
			...
			{#if hoveredNode}
				...
				{#if hoveredNode.details?.actions}
					<p>{hoveredNode.details.actions}</p>
				{:else if hoveredNode.details?.modules}
					<p>{hoveredNode.details.modules}</p>
				{:else if hoveredNode.details?.items}
					<p>{hoveredNode.details.items}</p>
				{/if}
			{:else if hoveredEdge}
